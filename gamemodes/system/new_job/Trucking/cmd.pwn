...
CMD:truckergo(playerid, params[])
{
    ...
    if(sscanf(params, "s[24]", type))
	{
		...
		SendServerMessage(playerid, "Lua chon: mission, buy, sell.");
		return 1;
	}
    if(PlayerInfo[playerid][pJob] != 20) return SendErrorMessage(playerid, "Ban khong phai la Trucker!!");
    ...
    else if(!strcmp(type, "sell", true))
	{
        ...
        {
            FactoryId = IsPlayerInFactory2(playerid);
            if(FactoryId == -1) return 1;
            SetPVarInt(playerid, "Sell_ProductID2", FactoryId);
            format(string, sizeof(string), "{FFFFFF}ID\t\tSan Pham\t\tGia");
            for(new i; i < strlen(FactoryExportData[FactoryId][ProductName]); i++)
            {
                ProductId = FactoryExportData[FactoryId][ProductName][i];
                format(string, sizeof(string),"%s\n%d\t\t%s\t\t$%d",string, i, ProductData[ProductId][ProductName], FactoryExportData[FactoryId][ProductPrice][i]);
            }
            Dialog_Show(playerid, DIALOG_SELLPRODUCT2,DIALOG_STYLE_TABLIST_HEADERS, "Danh Sach Cac San Pham", string, "Xac nhan", "<");
        }
    }
    return 1;
}