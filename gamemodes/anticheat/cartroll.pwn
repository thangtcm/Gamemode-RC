...
static bool:IsPlayerPaused_CT(playerid) 
...
hook OnPlayerStateChange(playerid, newstate, oldstate)
{
	switch(newstate)
	{
		...
		case 2:
		{
			...
			if(vehicleData[playerData[playerid][playerStateVehicle]][broadVehicleOwner] != INVALID_PLAYER_ID && vehicleData[playerData[playerid][playerStateVehicle]][broadVehicleOwner] != playerid && IsPlayerPaused_CT(vehicleData[playerData[playerid][playerStateVehicle]][broadVehicleOwner]))
			{
...
{	
	new reason[6][] = {
		...
		"Control Vehicle",
		"Fast Kick/Rotate Vehicle",
		"Illegally Driver",
		"Illegally Passenger",
		"Remote kick player from vehicle"
	};
	new mess[128];
	format(mess, sizeof(mess), "RCRP-AC: %s (PID: %d) co the dang su dung %s.", GetPlayerNameEx(playerid, false), playerid, reason[trolltype]);
	ABroadCast(COLOR_YELLOW, mess, 2);
	format(mess, sizeof(mess), "[RCRP-AC] %s (%d) da bi kick khoi may chu [Reason: RCRP-AC %s].", GetPlayerNameEx(playerid, false), playerid, reason[trolltype]);
